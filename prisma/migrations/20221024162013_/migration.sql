/*
  ...
  - You are about to drop the `bookstore_books` table. If the table is not empty, all the data it contains will be lost.
...
CREATE TABLE "bookstoreBooks" (
    ...
    CONSTRAINT "bookstoreBooks_bookstoreId_fkey" FOREIGN KEY ("bookstoreId") REFERENCES "bookstore" ("id") ON DELETE CASCADE ON UPDATE CASCADE,
    CONSTRAINT "bookstoreBooks_bookId_fkey" FOREIGN KEY ("bookId") REFERENCES "book" ("id") ON DELETE CASCADE ON UPDATE CASCADE
);
...
CREATE TABLE "new_rent_user_book" (
    ...
    CONSTRAINT "rent_user_book_bookstoreBooksId_fkey" FOREIGN KEY ("bookstoreBooksId") REFERENCES "bookstoreBooks" ("id") ON DELETE CASCADE ON UPDATE CASCADE
);
